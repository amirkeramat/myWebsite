export const projectItems = [
  {
    id: 1,
    title: "BlueBee",
    content:
      "A shopping webSite like digikala built with react JS,redux,tailwind CSS,styled-component,",
    href: "https://github.com/amirkeramat/digikala",
    desc: [{ id: 1, title: "JavaScript" }],
  },
  {
    id: 2,
    title: "MyPortfolio",
    content: "My Portfolio built with react JS,tailwind CSS,styled-component",
    href: "https://github.com/amirkeramat/myWebsite",
    desc: [{ id: 1, title: "JavaScript" }],
  },
  {
    id: 3,
    title: "Muvi",
    content:
      "Millions of movies, TV shows and people to discover. Explore now.",
    href: "https://github.com/amirkeramat/muvi",
    desc: [{ id: 1, title: "JavaScript" }],
  },
];

export const latestProjectsItems = [
  {
    id: 1,
    images: [
      { id: 1, src: "./bluebee.png" },
      { id: 2, src: "./bluebee2.png" },
    ],
    link: "https://amirkeramat.github.io/digikala/",
    title: "Shopping Website",
    desc: "a shopping website with real api and backend",
    tech: [
      { id: 1, title: "React Js" },
      { id: 2, title: "Redux" },
      { id: 3, title: "Tailwind CSS" },
      { id: 4, title: "Styled-component" },
    ],
    duration: "1 week",
    role: "Front-End Developer",
    live: 1,
    github: "https://github.com/amirkeramat/digikala",
  },
  {
    id: 2,
    images: [
      { id: 1, src: "./medical.png" },
      { id: 1, src: "./medical2.png" },
    ],
    link: "https://amirkeramat.github.io/medical/",
    title: "Medical Website",
    desc: "Medical Website for Hospital or Doctors",
    tech: [
      { id: 1, title: "React Js" },
      { id: 2, title: "Tailwind CSS" },
      { id: 3, title: "Json server" },
    ],
    duration: "2 weeks",
    role: "Front-End Developer",
    live: 1,
    github: "https://github.com/amirkeramat/medical",
  },
  {
    id: 3,
    images: [
      { id: 1, src: "./muvi.jpeg" },
      { id: 2, src: "./muvi-2.jpeg" },
    ],
    link: "https://amirkeramat.github.io/muvi/",
    title: "Muvi",
    desc: "Millions of movies, TV shows and people to discover. Explore now.",
    tech: [
      { id: 1, title: "React Js" },
      { id: 2, title: "Redux" },
      { id: 3, title: "Tailwind CSS" },
    ],
    duration: "10day",
    role: "Front-End Developer",
    live: 1,
    github: "https://github.com/amirkeramat/muvi",
  },
  {
    id: 4,
    images: [
      { id: 1, src: "./ecommerce-dashboard.png" },
      { id: 2, src: "./ecommerce-dashboard2.png" },
    ],
    link: "https://amirkeramat-ecommerce-store.vercel.app/",
    title: "Ecommerce-store",
    desc: "Ecommerce-store developed by nextJs13. ",
    tech: [
      { id: 1, title: "NextJs 13" },
      { id: 2, title: "React js" },
      { id: 3, title: "Tailwind CSS" },
      { id: 4, title: "Headless Ui" },
      { id: 4, title: "stripe" },
    ],
    duration: "10day",
    role: "Front-End Developer",
    live: 1,
    github: "https://github.com/amirkeramat/ecommerce-store",
  },
  {
    id: 5,
    images: [
      { id: 1, src: "./ecommerce-store.png" },
      { id: 2, src: "./ecommerce-store2.png" },
    ],
    link: "https://amirkeramat-ecommerce-dashboard.vercel.app/",
    title: "Ecommerce-dashboard",
    desc: "Admin panel for Ecommerce-store.you can create your special store with exclusive categories and filters ,banners,billboards ",
    tech: [
      { id: 1, title: "NextJs 13" },
      { id: 2, title: "React Js" },
      { id: 3, title: "Tailwind CSS" },
      { id: 4, title: "Prisma" },
      { id: 5, title: "Clerk" },
      { id: 6, title: "Zostand" },
      { id: 7, title: "Shadcn Ui" },
    ],
    duration: "10day",
    role: "Full stack Developer",
    live: 1,
    github: "https://github.com/amirkeramat/ecommerce-dashboard",
  },
  {
    id: 6,
    images: [
      { id: 1, src: "./text-me.png" },
      { id: 2, src: "./text-me2.png" },
    ],
    link: "https://text-me-iota.vercel.app/",
    title: "Realtime messenger",
    desc: "real time messenger project developed by nextJs13",
    tech: [
      { id: 1, title: "NextJs 13" },
      { id: 2, title: "React Js" },
      { id: 3, title: "Tailwind CSS" },
      { id: 4, title: "Prisma" },
      { id: 5, title: "Pusher" },
      { id: 6, title: "Zostand" },
      { id: 7, title: "NextAuth" },
    ],
    duration: "20day",
    role: "Full stack Developer",
    live: 1,
    github: "https://github.com/amirkeramat/realtime-messenger",
  },
];
