export const abilities = [
  {
    id: 1,
    title: "Front-End Ui/Ux",
    items: [
      { id: 1, title: "CSS", icon: "devicon-css3-plain-wordmark", rate: 4 },
      {
        id: 2,
        title: "Tailwind",
        icon: "devicon-tailwindcss-plain",
        rate: 4,
      },
      {
        id: 3,
        title: "Bootstrap",
        icon: "devicon-bootstrap-plain-wordmark",
        rate: 3,
      },
    ],
  },
  {
    id: 2,
    title: "Front-End Development",
    items: [
      { id: 1, title: "Html", icon: "devicon-html5-plain-wordmark", rate: 5 },
      { id: 2, title: "JS", icon: "devicon-javascript-plain", rate: 4 },
      { id: 3, title: "React JS", icon: "devicon-react-original", rate: 4 },
      { id: 4, title: "Redux", icon: "devicon-redux-original", rate: 4 },
      { id: 5, title: "Next JS", icon: "devicon-nextjs-original", rate: 2 },
    ],
  },
  {
    id: 3,
    title: "Databases",
    items: [
      {
        id: 1,
        title: "MongoDB",
        icon: "devicon-mongodb-plain-wordmark",
        rate: 4,
      },
      { id: 2, title: "MySQL", icon: "devicon-mysql-plain-wordmark", rate: 4 },
      {
        id: 3,
        title: "Azure",
        icon: "devicon-azure-plain-wordmark",
        rate: 4,
      },
    ],
  },
];

export const aboutMe = [
  {
    id: 1,
    title: "1.About Me",
    desc: "Im was born in 1996 in Iran-Tabriz. I Get my university degree in 2019 I started to learn Web development in 2020 and life is going on....",
  },
  {
    id: 2,
    title: "2.Interests and Hobbies",
    desc: "i love playing guitar and watching movies...and of course coding! and also i update my skills with new modern frameworks and im always be sure to increase my knowledge about web development.",
  },
];
